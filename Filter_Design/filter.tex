...
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/f_resp.png}
\caption{The Analog Low-Pass Frequency Response for $0.304  \leq \epsilon \leq 0.61$}
\label{fig:H_for_diff_eb}
\end{figure}

In \figref{fig:H_for_diff_eb} we can observe the equiripple behaviour in passband and monotonic behaviour in stopband. As the value of $\epsilon$ increases the value of $\vert H_{a,LP}(j\Omega_L)\vert$ decreases.  We choose $\epsilon = 0.4$  for our IIR filter design.  
\\
